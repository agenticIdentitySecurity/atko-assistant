...
The Frontier DB MCP server exposes four tools over the stdio transport. Claude selects and calls these tools based on the user's query.

| Tool | Purpose | Key Parameters |
|------|---------|----------------|
| `query_customers` | Filter customer records by email | `filter_email`, `limit` |
| `query_orders` | List orders, optionally by customer | `customer_id`, `limit` |
| `get_customer_with_orders` | Customer profile + order history | `customer_id` (required) |
| `search_products` | Full-text search on name/description | `search_term` (required), `limit` |

The database is pre-populated with sample data: 5 customers, 5 products, and 10 orders with line items.
...
### OIDC Application
The OAuth/OIDC app that consumers log into.
...
- **Scope**: `frontier:read`
- **Role**: Step 2 of token exchange — issues the consumer-scoped MCP token
...
### What the Agent Gets
- A short-lived access token scoped to `frontier:read`
- Only after Okta confirms the consumer's policy allows it
